...
MpiSy2sbContext<T>::MpiSy2sbContext(const MpiConfig& config, size_t matrix_n,
    ...
    : mpi_config(config),
      ...
      W_host(W),
      Y_host(Y) {
    util::MpiLogger::println("Process {} starting MpiSy2sbContext constructor", mpi_config.rank);
    ...
    util::MpiLogger::println("Process {} initializing GPU resources", mpi_config.rank);
    initGpuResources();
    
    util::MpiLogger::println("Process {} initializing communication", mpi_config.rank);
    initCommunication();
    
    util::MpiLogger::println("Process {} allocating GPU memory", mpi_config.rank);
    allocateGpuMemory();
    
    util::MpiLogger::println("Process {} completed MpiSy2sbContext constructor", mpi_config.rank);
}
...
void MpiSy2sbContext<T>::initCommunication() {
    util::MpiLogger::println("Process {} starting initCommunication", mpi_config.rank);
    ...
    util::MpiLogger::println("Process {} completed MPI_Allgather", mpi_config.rank);
    ...
    util::MpiLogger::println("Process {} completed main NCCL ID broadcast", mpi_config.rank);
    
    ncclResult_t nccl_result = ncclCommInitRank(&nccl_comm, mpi_config.size, 
                                               main_nccl_id, mpi_config.rank);
    if (nccl_result != ncclSuccess) {
        throw std::runtime_error(fmt::format("Main NCCL initialization failed: {}",
                                             ncclGetErrorString(nccl_result)));
    }

    util::MpiLogger::println("Process {} completed main NCCL comm init", mpi_config.rank);
    ...
    util::MpiLogger::println("Process {} starting sub-group creation loop", mpi_config.rank);
    
    for (int start_rank = 0; start_rank < mpi_config.size; start_rank++) {
        util::MpiLogger::println("Process {} processing sub-group starting from rank {}", 
                               mpi_config.rank, start_rank);
        
        // 所有进程都参与MPI_Comm_split，但只有部分进程会被分配到有效的通信器
        int color = (mpi_config.rank >= start_rank) ? start_rank : MPI_UNDEFINED;
        
        util::MpiLogger::println("Process {} calling MPI_Comm_split for group {} with color={}", 
                               mpi_config.rank, start_rank, color);
        
        MPI_Comm_split(MPI_COMM_WORLD, color, mpi_config.rank, &sub_mpi_comms[start_rank]);
        
        util::MpiLogger::println("Process {} completed MPI_Comm_split for group {}", 
                               mpi_config.rank, start_rank);
        ...
        if (mpi_config.rank >= start_rank) {
            ...
            util::MpiLogger::println("Process {} creating sub-group {}: size={}, sub_rank={}", 
            ...
            if (sub_group_size > 1) {
                ...
                if (mpi_config.rank == start_rank) {
                    ...
                    util::MpiLogger::println("Process {} generated NCCL ID for group {}", 
                                           mpi_config.rank, start_rank);
                }
                
                // 在子组内广播NCCL ID
                util::MpiLogger::println("Process {} starting NCCL ID broadcast for group {}", 
                                       mpi_config.rank, start_rank);
                MPI_Bcast(&sub_nccl_id, sizeof(sub_nccl_id), MPI_BYTE, 0, sub_mpi_comms[start_rank]);
                
                util::MpiLogger::println("Process {} completed sub-group NCCL ID broadcast for group {}", 
                                       mpi_config.rank, start_rank);
                
                // 初始化子组NCCL通信器
                util::MpiLogger::println("Process {} initializing NCCL comm for group {}", 
                                       mpi_config.rank, start_rank);
                ncclResult_t sub_result = ncclCommInitRank(&sub_comm_groups[start_rank], 
                                                          sub_group_size, sub_nccl_id, sub_rank);
                
                if (sub_result != ncclSuccess) {
                    throw std::runtime_error(fmt::format("Sub NCCL group {} initialization failed: {}",
                                                         start_rank, ncclGetErrorString(sub_result)));
                }
                
                util::MpiLogger::println("Process {} completed NCCL initialization for group {}", 
                                       mpi_config.rank, start_rank);
            } else {
                // 单进程组：不需要NCCL通信器
                sub_comm_groups[start_rank] = nullptr;
                util::MpiLogger::println("Process {} skipping NCCL creation for single-process group {}", 
                                       mpi_config.rank, start_rank);
            }
            
            util::MpiLogger::println("Process {} joined sub-group starting from rank {}, sub_rank={}, group_size={}", 
                                   mpi_config.rank, start_rank, sub_rank, sub_group_size);
        } else {
            sub_comm_groups[start_rank] = nullptr;
            // sub_mpi_comms[start_rank] 已经在MPI_Comm_split中设置为MPI_COMM_NULL
            util::MpiLogger::println("Process {} not participating in sub-group {}", 
                                   mpi_config.rank, start_rank);
        }
    }
    
    util::MpiLogger::println("Process {} completed initCommunication", mpi_config.rank);
}
...
void sy2sb_recursive_mpi(size_t recursive_depth,
    ...
    util::MpiLogger::println(
        ...
        "= {}",
        ctx.mpi_config.rank, recursive_depth, recursive_offset_finished, ctx.n, ctx.nb,
        recursive_offset_finished, ctx.n <= ctx.nb + recursive_offset_finished);
    ...
    for (size_t i = ctx.b;
        ...
        util::MpiLogger::println("Process {} entering panel loop: i={}, global_panel_col={}, panel_m={}, panel_n={}", 
        ...
        util::MpiLogger::println("Process {} calling performPanelQR for global_panel_col={}", 
        ...
        hierarchicalBarrier(ctx, owner_rank, fmt::format("phase-{}-complete", current_phase));
    }
...
void exchangeDataMPI(matrix_ops::mpi::MpiSy2sbContext<T>& ctx,
    ...
    util::MpiLogger::println("Using sub-communication group starting from rank {}, current process participating: {}", 
...
void performInterRecursiveSyr2k(matrix_ops::mpi::MpiSy2sbContext<T>& ctx,
    ...
    // 这样可以减少通信延迟，提高带宽利用率
    for (int group_start = 0; group_start < ctx.mpi_config.size; group_start++) {
        if (ctx.mpi_config.rank >= group_start && 
            ctx.sub_comm_groups[group_start] != nullptr) {
            
            util::MpiLogger::println("Process {} participating in sub-group {} AllReduce", 
    ...
    util::MpiLogger::println("Process {} completed syr2k with hierarchical communication", 
                           ctx.mpi_config.rank);
}
...
void pipelineParallelScheduler(matrix_ops::mpi::MpiSy2sbContext<T>& ctx,
    ...
    if (ctx.mpi_config.rank < min_participating_rank) {
        ...
        util::MpiLogger::println("Process {} starting next phase while others finish current phase", 
                               ctx.mpi_config.rank);
...
void hierarchicalBarrier(matrix_ops::mpi::MpiSy2sbContext<T>& ctx, 
                         ...
                         const std::string& operation_name) {
    // 添加调试信息
    util::MpiLogger::println("Process {} checking barrier for {}: min_rank={}, rank>={}, size={}, comm_null={}", 
                           ctx.mpi_config.rank, operation_name, min_participating_rank,
                           ctx.mpi_config.rank >= min_participating_rank,
                           ctx.sub_mpi_comms.size(),
                           min_participating_rank < ctx.sub_mpi_comms.size() ? 
                           (ctx.sub_mpi_comms[min_participating_rank] == MPI_COMM_NULL) : true);
    ...
    if (ctx.mpi_config.rank >= min_participating_rank && 
        ...
        util::MpiLogger::println("Process {} syncing with sub-group [{},...,{}] after {}", 
                               ctx.mpi_config.rank, min_participating_rank, 
        ...
        // 早完成的进程可以继续下一阶段工作
        util::MpiLogger::println("Process {} skipping barrier for {} (already finished this phase)", 
                               ctx.mpi_config.rank, operation_name);
    } else {
        // 调试：其他情况
        util::MpiLogger::println("Process {} cannot participate in barrier for {} (comm not available)", 
                               ctx.mpi_config.rank, operation_name);
    }
}
...
void sy2sb(const MpiConfig& mpi_config, size_t n, T* A, size_t lda, T* W,
    ...
    util::MpiLogger::println("Process {} entering sy2sb function", mpi_config.rank);
    ...
    util::MpiLogger::println("Process {} creating MPI sy2sb context", mpi_config.rank);
    
    // 创建 MPI sy2sb 上下文
    MpiSy2sbContext<T> ctx(mpi_config, n, A, lda, W, ldw, Y, ldy, nb, b);

    util::MpiLogger::println("Process {} calling recursive implementation", mpi_config.rank);
    
    // 调用递归实现
    internal::sy2sb_recursive_mpi<T>(0, ctx);

    util::MpiLogger::println("Process {} completed recursive implementation", mpi_config.rank);

    // 最终同步：确保所有进程完成
    // 使用层次化同步：从小组到大组逐步同步，最后全局同步
    util::MpiLogger::println("Process {} starting final hierarchical synchronization", mpi_config.rank);
    
    // 先在各自的子组内同步
    for (int group_start = mpi_config.size - 1; group_start >= 0; group_start--) {
        if (mpi_config.rank >= group_start && 
            group_start < ctx.sub_mpi_comms.size() &&
            ctx.sub_mpi_comms[group_start] != MPI_COMM_NULL) {
            util::MpiLogger::println("Process {} syncing with sub-group [{},...,{}]", 
    ...
    // 最后全局同步确保所有进程完成
    util::MpiLogger::println("Process {} final global synchronization", mpi_config.rank);
